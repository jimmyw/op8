...
function chip8(program) {
  ...
  this.pc = 0x200;                  // 0x200 is where our program is loaded.
  this.speed = 0;                   // ms. Timing < 100 makes 10 instruction or until 1 draw call each, timing >= 100 is just one instruction
  this.opcode = 0;                  // This is the current executing instruction.
  this.I = 0;                       // Indexregister, 16 bits wide
  this.c = 0;                       // Instruction counter, just for debug.
  this.sp = 0;                      // Stackpointer
  this.V = new Uint8Array(16);      // 16 - 8bit registers
  this.S = new Uint16Array(16);     // 16 - 16bit stack for return adresses
  this.G = new Uint8Array(64 * 32); // Graphic buffer, 64x32 pixels 8-bit
  this.M = new Uint8Array(4096);    // 4k 8-bit memory
  this.timer = 0;                   // This is set by a instrucion, and is unixtime.
  this.sound_timer = 0;             // This is just set, but sound are not played yet.
  this.keyboard = new Uint8Array(16); // This is a map of our keyboard. 1 means key is pressed. Multible keys can be pressed at once.
  this.zoom = 8;                    // This is the number of screen pixels, one super8 pixel is scaled up to.
